...
public class Parametros extends HttpServlet {
    ...
    /*private Enumeration<String> valores;*/
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Configuración</title>"
                    + "<style type=\"text/css\" media=\"screen\">"
                    + "div{width:40%;margin:5% 30% auto 30%;border:1px solid blue;text-align:center;}</style>");            
            out.println("</head>");
            out.println("<body><div>"
                    + "<h1>Parámetros de Configuración</h1>");
            
            while(parametros.hasMoreElements()){
                out.print("<p>Parámetro: "+parametros.nextElement()+"</p>");
            }
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        parametros = getInitParameterNames();
        /*while(parametros.hasMoreElements()){
